...
use specs::arch::{
    ContextSwitch, ContextSwitchError, StackGuard, 
    StackGuardContext, StackGuardError, StackGuardMode, 
    StackGuardState,
};
...
impl SchedulerPolicy for MockScheduler {
    fn register_task(&mut self, _task_id: TaskId, _priority: TaskPriority) -> Result<(), KernelError> {
        Ok(())
    }

    fn enqueue_runnable(&mut self, _task_id: TaskId, _priority: TaskPriority) -> Result<(), KernelError> {
        Ok(())
    }
